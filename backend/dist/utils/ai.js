...
export async function parseResume(pdfText) {
...
export async function generateEmail(prompt, candidateName) {
    const response = await getAi().chat.completions.create({
        model,
        messages: [
            {
...
export async function evaluateCandidateMatch(resumeData, jobText) {
    const response = await getAi().chat.completions.create({
        model,
        messages: [
            {
...
export async function semanticSearchCandidates(prompt, candidatesList) {
    const response = await getAi().chat.completions.create({
        model,
        messages: [
            {
